  ...
  describe('Search', function() {
    ...
    it('should  validates that all documents, limited by a specified number, that were published on a certain date', function(done) {
      dms.getDocByDate(dateGetter(), 2, function(err, docs) {
        ...
        expect(docs[0].docTitle).toBe('The epic battle of world changers');
        expect(docs[0].datePublished).toBeDefined();
        expect(docs[0].accessTo).toEqual('manager');
        expect(docs[1].docTitle).toBe('The beautiful ones are not yet born');
        expect(docs[1].datePublished).toBeDefined();
        expect(docs[1].accessTo).toEqual('admin');
        done();
      });
    });
  });
